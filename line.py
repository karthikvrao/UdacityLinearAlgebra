...
class Line(object):
    ...
    def __eq__(self, other):
        ...
        # Base points of each line
        x0 = self.basepoint
        y0 = other.basepoint

        # Get vector connecting two line's basepoints
        basepoint_difference = x0.minus(y0)

        n = self.normal_vector
        # Check if the connecting vector is orthogonal to either line's normal vector
    ...
    def intersection_with(self, other):
        try:
            ...
            one_by_denominator = Decimal(1)/(A * D - B * C)

            return Vector([x_numerator, y_numerator]).times_scalar(one_by_denominator)
